...
def get_data(cover_str, p_idx=None):
    data_lines = DATA_LINES if p_idx is None else [f'{p_idx:05d}']
    data_dict = {}
...
if __name__ == '__main__':
    ...
    data_dict = get_data(args.cover_type, p_idx=args.p_idx)
    name = f'{args.cover_type}_{args.p_idx:03}_{args.pose_idx}'

    ref = np.ones((520, 412, 3), dtype=np.uint8) * 255
    cv2.putText(ref, 'SLP RGB unavailable', (95, 30), 0, 0.85, (0, 0, 0), 1, cv2.LINE_AA)
    ...
    if args.viz_type == 'video':
        ...
        for render in renders:
            ...
            render = cv2.putText(render, 'SLP RGB unavailable', (40, 30), 0, 0.85, (0, 0, 0), 1, cv2.LINE_AA)
            render = cv2.putText(render, 'SLP RGB unavailable', (245, 30), 0, 0.85, (0, 0, 0), 1, cv2.LINE_AA)
