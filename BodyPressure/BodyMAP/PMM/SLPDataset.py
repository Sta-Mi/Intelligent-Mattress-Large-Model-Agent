...
class SLPDataset():
    ...
    def __init__(self, data_path):
        ...
        self.pressure_data_dir = os.path.join(data_path, 'slp_real_cleaned')
    ...
    def prepare_dataset(self, data_lines, cover_str, load_verts=False, for_infer=False, train_on_real='all'):
        ...
        slp_T_cam_all = np.load(os.path.join(self.slp_real_cleaned_data_dir, 'slp_T_cam_0to102.npy'), allow_pickle=True)
        depth_all = np.load(os.path.join(self.slp_real_cleaned_data_dir, f'depth_{cover_str}_cleaned_0to102.npy'), allow_pickle=True)
        pressure_all = np.load(os.path.join(self.slp_real_cleaned_data_dir, 'pressure_recon_Pplus_gt_0to102.npy'))
        ...
        for person_num_str in data_lines:
            person_idx = int(person_num_str) - 1
            ...
            for i in range(start_pose, stop_pose, 1):
                ...
                gender_value = pose_data.get('gender', 'female')
                ...
                pressure_image = np.clip(pressure_all[person_idx, i].astype(np.float32), 0, 100)

                depth_image = depth_all[person_idx, i]
                depth_image[127:, :] = 0.0

                pose_data['slp_T_cam'] = np.array(slp_T_cam_all[person_idx, i])
                ...
                body_shape = pose_data['betas']
                joint_angles = np.array(list(pose_data['global_orient']) + list(pose_data['body_pose']))
                root_xyz_shift = pose_data['O_T_slp'] + pose_data['slp_T_cam'] + pose_data['cam_T_Bo'] + pose_data['Bo_T_Br']
                root_xyz_shift[1:] *= -1
